...
def harmony_mtx(source_mtx: np.ndarray, target_mtx: np.ndarray, n_pca=50, **kwargs) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    Args:
        ...
        n_pca: Harmony runs on a PCA(n_pca) embedding (default 50). Pass a falsy value
               (None/False/0) to run Harmony directly on the raw features instead.
    ...
    """
    # Info message on the root logger (not harmonypy's), so it shows by default.
    logging.info(
    ...
    if n_pca:
        ...
        _p = PCA(n_components=min(int(n_pca), source_mtx.shape[1]), random_state=0).fit(np.vstack([source_mtx, target_mtx]))
        source_mtx, target_mtx = _p.transform(source_mtx), _p.transform(target_mtx)
...
def harmony_multi_mtx(blocks, n_pca=50, **kwargs):
    """Multi-batch Harmony: jointly integrate a list of matrices (one per batch/dataset).

    The multi-dataset counterpart of ``harmony_mtx``. Runs Harmony on a shared PCA(n_pca)
    embedding of all blocks stacked together (default 50, matching ``harmony_mtx``; pass a
    falsy value to run directly on the raw features), then splits the corrected matrix back
    into one array per input block. Shares the exact same ``harmonize_numpy`` / ``_run_harmony``
    core as the pairwise route, so pair and multi differ only in how many blocks are joined.

    Args:
        blocks: list of arrays, each (n_i, n_features); all must share n_features.
        n_pca: PCA dimensionality before Harmony (default 50; falsy -> no PCA).
        **kwargs: forwarded to the core Harmony algorithm (theta, lamb, sigma, ...).

    Returns:
        list of corrected arrays, one per input block (row order preserved).
    """
    if len(blocks) < 2:
        raise ValueError("harmony_multi_mtx needs at least two blocks.")
    ...
    batch_labels = np.concatenate([[f"batch{i}"] * s for i, s in enumerate(sizes)])
    if n_pca:
        from sklearn.decomposition import PCA
        _p = PCA(n_components=min(int(n_pca), combined.shape[1]), random_state=0).fit(combined)
        combined = _p.transform(combined)
    corrected = harmonize_numpy(combined, batch_labels, **kwargs)
    return list(np.split(corrected, np.cumsum(sizes)[:-1]))
