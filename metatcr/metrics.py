...
def compute_jsd(mtx1: np.ndarray, mtx2: np.ndarray, block: int = 256) -> float:
    """
    ...
    Args:
        ...
        block (int): Row-block size over mtx1; bounds the peak memory of the
                     pairwise mixture tensor to block * n_samples2 * n_features.

    Returns:
        float: The average JSD between the two matrices.

    Note:
        Accelerated, computationally equivalent implementation of the pairwise
        Jensen-Shannon divergence; `_jensen_shannon_divergence` is the scalar
        single-pair form. `scipy.special.rel_entr` is the kernel
        `scipy.stats.entropy` uses internally (with per-argument renormalization).
        The `block` argument bounds peak memory of the pairwise mixture tensor.
    """
    if mtx1.ndim == 1: mtx1 = mtx1.reshape(1, -1)
    if mtx2.ndim == 1: mtx2 = mtx2.reshape(1, -1)

    P = _row_probabilities(mtx1)
    Q = _row_probabilities(mtx2)
    # scipy.stats.entropy renormalizes each argument; the first arguments are
    # per-row and pair-independent, so renormalize them once.
    P_hat = P / P.sum(axis=1, keepdims=True)
    Q_hat = Q / Q.sum(axis=1, keepdims=True)

    n1, n2 = P.shape[0], Q.shape[0]
    total = 0.0
    for s in range(0, n1, block):
        Pb = P[s:s + block]                                  # (b, d) for the mixture
        Pbh = P_hat[s:s + block][:, None, :]                 # (b, 1, d)
        M = 0.5 * (Pb[:, None, :] + Q[None, :, :])           # (b, n2, d) pairwise mixture
        Mn = M / M.sum(axis=2, keepdims=True)                # renormalized mixture
        kl_pm = rel_entr(Pbh, Mn).sum(axis=2)                # (b, n2)
        kl_qm = rel_entr(Q_hat[None, :, :], Mn).sum(axis=2)  # (b, n2)
        total += float((0.5 * (kl_pm + kl_qm)).sum())
    return total / (n1 * n2)


def _row_probabilities(mtx: np.ndarray) -> np.ndarray:
    """Row-wise clip-to-nonnegative then L1 normalize, with EPS in the denominator."""
    clipped = np.maximum(mtx, 0.0)
    return clipped / (clipped.sum(axis=1, keepdims=True) + EPS)


def _mean_directed_ce(P: np.ndarray, Q: np.ndarray) -> float:
    """Mean over all row pairs of the cross-entropy H(P_i, Q_j) = -sum_k P_i,k log(Q_j,k + EPS),
    with P and Q already row-probability matrices. Uses the bilinear identity
    mean_{i,j} H(P_i, Q_j) = -(P.sum(0) . log(Q+EPS).sum(0)) / (n_p * n_q)."""
    n_p, n_q = P.shape[0], Q.shape[0]
    log_q_colsum = np.log(Q + EPS).sum(axis=0)            # (n_features,)
    return -float(P.sum(axis=0) @ log_q_colsum) / (n_p * n_q)


# compute_ce supports symmetric (default) and asymmetric calculation.
def compute_ce(mtx1: np.ndarray, mtx2: np.ndarray, symmetric: bool = True) -> float:
    ...
    Note:
        Accelerated, computationally equivalent implementation of the pairwise
        cross-entropy; `_cross_entropy` is the scalar single-pair form.
    """
    if mtx1.ndim == 1: mtx1 = mtx1.reshape(1, -1)
    if mtx2.ndim == 1: mtx2 = mtx2.reshape(1, -1)
    ...
    P = _row_probabilities(mtx1)
    Q = _row_probabilities(mtx2)
    ce_12 = _mean_directed_ce(P, Q)          # source=mtx1, target=mtx2
    if not symmetric:
        return ce_12
    ce_21 = _mean_directed_ce(Q, P)          # source=mtx2, target=mtx1
