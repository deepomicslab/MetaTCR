...
def resolve_faiss_gpu(device=None):
    '''
    Resolve the faiss GPU ordinal to run on.
    :param device: int or None. If None, falls back to the METATCR_FAISS_GPU
        env var (default 0). The ordinal is relative to the GPUs visible to the
        process, so it composes with CUDA_VISIBLE_DEVICES.
    :return: int
    '''
    if device is None:
        device = int(os.environ.get("METATCR_FAISS_GPU", 0))
    return int(device)

def kmeans_clustering(X, cluster_num = 50, device = None, seed = None):
    '''
    :param X: np.array, shape = (n_samples, d)
    :param cluster_num: int, number of clusters
    :param device: int or None, faiss GPU ordinal (see resolve_faiss_gpu). Set it
        (or the METATCR_FAISS_GPU env var) to run on e.g. card 2 or 3.
    :param seed: int or None, faiss k-means seed. None keeps the faiss default.
    ...
    flat_config.device = resolve_faiss_gpu(device)
...
def kmeans_traverse_k(X, cluster_range, device = None):  ## X is a 2-dim np.array, cluster_range is a list
    ...
    for cluster_num in cluster_range:
        ...
        labels, centroids, last_stat = kmeans_clustering(X, cluster_num, device=device)
