...
def csv2meta_feature(filelist, centroids, emb_model, tcr_col='full_seq', freq_col='count (templates/reads)', seq_batch_size=2048):
    """
    ...
    with tqdm.tqdm(total=len(filelist), desc="Processing batch") as pbar:
        for idx, file in enumerate(filelist):
            ...
            try:
                ...
                X = seqlist2ebd(seqlist, emb_model, keep_pbar=False, batch_size=seq_batch_size)
                ...
                # cluster ids are small indices (0..len(centroids)-1); store as int16 to save space
                all_clst_ids.append(np.asarray(seqlabels, dtype=np.int16))
...
def process_dataset_to_metavec(filelist, all_smplabels, centroids, emb_model, dataset_name, out_dir, batch_size=100, seq_batch_size=2048):
    """
    ...
    for i in range(start_index, len(filelist), batch_size):
        ...
        all_clst_ids, diversity_mtx, abundance_mtx = csv2meta_feature(batch_files, centroids, emb_model, seq_batch_size=seq_batch_size)
